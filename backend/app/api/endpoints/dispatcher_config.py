...
async def list_dispatcher_configs(skip: int = 0, limit: int = 20, db: AsyncSession = Depends(get_db)):
    ...
    for c in configs:
        ...
        resp_configs.append(DispatcherConfigResponse(
            ...
            outbound_mode=c.outbound_mode or "agent",
            ai_formulation_prompt=c.ai_formulation_prompt,
...
async def create_dispatcher_config(config: DispatcherConfigCreate, db: AsyncSession = Depends(get_db)):
    ...
    db_config = DispatcherConfig(
        ...
        ai_formulation_prompt=config.ai_formulation_prompt,
    ...
    return DispatcherConfigResponse(
        ...
        outbound_mode=db_config.outbound_mode or "agent",
        ai_formulation_prompt=db_config.ai_formulation_prompt,
