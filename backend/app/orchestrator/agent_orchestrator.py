...
"""
    ...
    async def _invoke_collaborator(
        ...
        # ── GENERIC FAST DISPATCHER: Activated if agent config contains "fast_dispatcher": true or "jev": true ──
        ...
        if isinstance(raw_agent_config, dict) and (raw_agent_config.get("fast_dispatcher") is True or raw_agent_config.get("jev") is True):
            try:
                from app.services.jev_dispatcher_service import JevDispatcherService
                dispatcher = JevDispatcherService(db=self.db, context_data=context_data)
                disp_res = await dispatcher.dispatch(
                    agent=agent,
                    message=message,
                ...
                if disp_res is not None:
                    print(f"[Orchestrator] ⚡ Collaborator '{agent.name}' resolved via Dynamic Dispatcher")
                    return (agent.name, disp_res)
            except Exception as disp_err:
                import traceback
                print(f"[Orchestrator] ⚠️ Error in dynamic dispatch for '{agent.name}': {disp_err}")
                traceback.print_exc()
        ...
        if getattr(agent, "is_orchestrator", False) and getattr(agent, "collaboration_enabled", False):
            try:
                ...
                if nested_agent_with_settings:
                    collab_tools, mandatory_instructions, _, _, _, all_collaborators = await _build_collaborator_tools(
