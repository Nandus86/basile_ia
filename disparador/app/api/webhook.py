...
async def receive_dispatch(
    ...
    try:
        # Validate payload
        try:
            payload = DispatchPayload(**body_json)
            log_entry.contact_count = len(payload.contacts)
            await db.commit()
        ...
        log_entry.status_code = he.status_code
        ...
        log_entry.error_message = str(he.detail)
        log_entry.duration_ms = int((time.time() - start_time) * 1000)
        await db.commit()
        raise
